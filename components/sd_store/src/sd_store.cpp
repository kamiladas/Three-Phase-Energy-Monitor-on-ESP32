...
constexpr char kYearHistoryPath[]="/sdcard/qemu_hourly_year_v2.bin";
constexpr char kYearHistoryMarker[]="/sdcard/.qemu_hourly_year_v2";
constexpr std::int64_t kYearHistoryStart=1756850400;constexpr std::int64_t kYearHistoryEnd=1788386400;
struct SimHistoryRecord{std::int64_t epoch_s;float energy_kwh[3];float active_power_w[3];float frequency_hz[3];};
...
esp_err_t sd_store_aggregate_utc(std::int64_t from,std::int64_t to,std::size_t count,SdAggregationResolution resolution,SdEnergyAggregate& out){
    ...
    while(std::fgets(line,sizeof(line),f)){
        char*ctx=nullptr;char*token=strtok_r(line,",",&ctx);int col=0;std::uint64_t sequence=0;std::int64_t epoch=0;double imported[3]{},power[3]{},frequency[3]{};
        while(token){if(col==1)sequence=std::strtoull(token,nullptr,10);else if(col==4)power[0]=std::strtod(token,nullptr);else if(col==8)frequency[0]=std::strtod(token,nullptr);else if(col==9)imported[0]=std::strtod(token,nullptr);else if(col==14)power[1]=std::strtod(token,nullptr);else if(col==18)frequency[1]=std::strtod(token,nullptr);else if(col==19)imported[1]=std::strtod(token,nullptr);else if(col==24)power[2]=std::strtod(token,nullptr);else if(col==28)frequency[2]=std::strtod(token,nullptr);else if(col==29)imported[2]=std::strtod(token,nullptr);else if(col==33)epoch=std::strtoll(token,nullptr,10);token=strtok_r(nullptr,",",&ctx);++col;}
        if(epoch<=0)continue;
        const bool continuous=previous_epoch>0&&epoch>=previous_epoch&&sequence>previous_sequence;
        if(epoch>=from&&epoch<to){auto&b=out.buckets[bucket_index(epoch)];for(int n=0;n<3;n++){b.active_power_w[n]+=power[n];b.frequency_hz[n]+=frequency[n];if(continuous){const double delta=imported[n]-previous_import[n];if(delta>=0.0)b.energy_kwh[n]+=delta/1000.0;}}b.measurement_count++;b.present=true;}
        previous_epoch=epoch;previous_sequence=sequence;for(int n=0;n<3;n++)previous_import[n]=imported[n];
    }
    std::fclose(f);
    for(std::size_t i=0;i<count;i++){auto&b=out.buckets[i];if(!b.measurement_count)continue;for(int n=0;n<3;n++){b.active_power_w[n]/=b.measurement_count;b.frequency_hz[n]/=b.measurement_count;}}
    return ESP_OK;
}
}
